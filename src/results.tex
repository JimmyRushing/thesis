...
As outlined in \autoref{kmcanalysis}, from the difference in \glsxtrshort{msd} bewteeen these sets of charge carriers the
...
temperature, 15 \glsxtrshort{kmc} simulations from 100K to 800k were run on the benchmark P3HT crystalline morphology. It is clear from the results in figure \ref{TEMP}(A) that the mobilities trend upward with temperature.
...
\subsection{\glsxtrshort{msd} (lifetimes)}

The \glsxtrshort{kmc} algorithm allows an explicit calculation of the \glsxtrshort{msd} across a large number of 
particles in the system. Repeating along relevant time scales for 
charge transport, the slope of this relationship
can be estimated and related to to the 3D diffusion coefficient as discussed in \autoref{kmcanalysis}. Mobility is obtained using \autoref{einstein}, the groundwork for which Einstein derived in his doctoral dissertation.

Including a \gls{msd} for lifetime in the ballistic transport timescale can effect the linear regression
estimate of the slope of the \gls{msd} as time goes to inifinity.
\cite{Maginn2018}. Estimating an upperbound for lifetimes such that
we can estimate the slope of the \glsxtrshort{msd} as time goes to infinity can be messy. In real systems, free chrage
...
It is advised that a preanalysis to determine where a given morphology's \glsxtrshort{msd} will converge be performed so as
...
squared displacement averaged over 1000 holes at $10^{-9}s$ and again $10^{-10}s$ the slope of the \glsxtrshort{msd} can be
...
With the slope between \glsxtrshort{msd} at the first lifetime and at the second lifetime proving an estimate of the slope
of the \glsxtrshort{msd}, 6 simulations we run with progressively shorter first lifetimes. That is, the second lifetime was
