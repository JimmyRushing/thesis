...
and rotated up to 180 degrees about its y-axis and z-axis. Distances between atoms less
than 3 angstroms were disallowed as distances shorter than this are considered
unphysical.
...
The data resulting from this experiment, $5680$ oreintations of interacting non-nonded thiophenes and the
correspinding $T_{ij}$ between them, provide evidence that MorphCT is ratioanlly capturing the orbital
...
Beyond these trends, the absolute value of these calculations match closely those
calculated using more rigerous ab initio DFT \cite{Lan2008}, where realistic distances between thiophene rings in lamaeler P3HT
crystals between $3.8\AA$ and $4 \AA$ gives $T_{ij}$ values between $0.07eV$ and $0.1eV$. 
...
the corresponing wait time for a charge to hop between to thiophenes in these oreintations is $\tau = 2
\cdot 10^{-14}s$ and $\tau = 2 \cdot 10^{-7}s$. With latter being two orders of magnitude slower than the
lifetimes allowed in our KMC simulations, a hop would be highly unfavored in this oreintations. 
The average of our
...
\end{subfigure}
    \caption{ABOVE: The orientations with the hightest $T_{ij}$ on the left and
    the lowest on the right. BELOW: Individual dots correspond to $5680$ 
    distinct rotatianal and translational
    combination of the upper
    thiophene to the reference thiophene.
    Dots are colored based on the $T_{ij}$ between thiophenes for the given x-axis, z-axis, and center to
    center distance to the reference thiophene. }
\label{TIplots}
\end{figure}
...
\hline\hline %inserts double horizontal lines
  & Amorphous & Semi-crystalline & Crystalline \\ [0.5ex] % inserts table
%heading
\hline % inserts single horizontal line
    Miller 2018  \cite{Miller2018} & $(1.085 \pm 0.006)\cdot 10^{-1}$ & $(0.156 \pm 0.003)\cdot 10^{-1}$ & $(1.23 \pm 0.01)\cdot 10^{-1}$ \\ % inserting body of the table
This work & $(1.8074 \pm 0.0001)\cdot 10^{-3}$ & $(1.4751 \pm 0.0001)\cdot 10^{-3}$ & $(2.061 \pm 0.001)\cdot 10^{-1}$  \\ [1ex] % [1ex] adds vertical space
