...
molecules decreasing with molecular length. Fused-ring geometries are of particlular interest for accepter
...
Consistant with other investigations, our INDO implemention estimates the homo-lumo gap to be above the 
DFT estimated value by around roughly a factor of 2 \cite{Gorelsky2001} \cite{Gorelsky2016}.
...
With each pair requiring a relatively costly QCC, after narrowing down the chromophore pairs with voronoi, it is 
computationally preferable to calculate the distances between all pairs and remove neighbors more than dcut
apart. We can see in table \ref{table:dcut-sense}, that with dcut = 10 we get comparable mobilities to the scut 12 simulation with $10^5$ less
pairs.  

The optimal dcut will vary depending on the material and before doing large sweeping analysis on a new
