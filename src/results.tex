...
The performance of morphct at the quantum chemical level is explored \ej{Finish this thought.}

Sensitivity testing on benchmark P3HT morphology provided by the Evan and matty work \ej{Cite}can was finegrained and
used to compare to previous mobility results as well as to test the sensitivity of the algorithm to
reorganization energy, dcut, MSD calculation, and simulation temperature. \ej{The bones are here, but a few clearer sentences in the intro about how this chapter is about (a) comparing your new ways against the old ways, and (b) doing a sensitivity analysis.}

On the spot \ej{What's on-the-spot mean here?} MD simulation of 1000
molecules was performed for testing morph another benchmark OPV material. Janks/mattys \ej{cite} work has shown that taking
...
hopping model to ITIC other small molecules is even more justified \ej{why?}. Knowing that the this molecule has
...
Delineating chromophores within a morphology needs to \ej{needs to, or can be?} be justified by DFT or experiment and balanced against
...
\ej{A better sentence structure might be something like ``To evaluate pySCF's suitability for X we perform two sets of experiments: (1) Comparing the HOMO-LUMO gap against Y, and (2) comparing the electronic overlap ($T_{ij}$) from the dimer splitting method against Z (ORCA calcs?).''}
...
\ej{one question that pops up right away is why look at these fused ring gaps for validation versus other things? Is it just that these papers exist?}
...
factor of 2-3 found in other investigations \cite{Gorelsky2001} \cite{Gorelsky2016}. \ej{Is this good? Bad?}
...
\ej{Changing section heading: This is a section about how sensitive charge transport is to various inputs. Not everyone has the same mapping from MorphCT->Charge Transport that you have in your head.}
