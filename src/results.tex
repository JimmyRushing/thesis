...
At the level of a single chromophore, we first recreate the well studied phenomena of the homo-lumo gap of
molecules decreasing with molecular length. 
This phenomenon has been studied for fused ring molecules using experiment and DFT methods \cite{Arago2010}.
...
\begin{figure}
  \center
  \includegraphics[width = \textwidth]{figures/fused-ring-figure.png}
  \caption{Fused Thiophene Experimental results.}
  \label{fig:fused}
\end{figure}
\begin{figure}
  \center
  \includegraphics[width = 0.5\textwidth]{figures/thiophene.png}
  \caption{Two thiophenes}
  \label{fig:thiophene}
\end{figure}
Using Mbuild, a reference thiophene is placed in the xy-plane
with the y-axis running through the the sulfur atom and thiophened center mass placed at the origin. 
A second thiophene is instantiated as in figure \ref{ref:thiophene}
...
The $|T_{ij}|$ resulting from 4300 unique orientations are shown in figure \ref{fig:TI}. The figure shows that, as expected, a
decrease in 
...
a rotation about the x-axis oreints the sulfer downward, resulting in a smaller sulfer-to-sulfler distance and a greater $|T_{ij}|$. 
