...
performance of the PySCF dimer calculations outlined in \autoref{methods}. 
...
PySCF into MorphCT properly, and that the quantities produced comport with the physics of these systems.
...
morphology to KMC predicted charge mobility. We use this opportunity to explore the TRUEness of MorphCT as
...
exploratory experiments meant to evaluate pySCF's suitabilitity for performing these duties:
(1) we compare frontier molecular energies of single chromophores given by pySCF to those
given by more rigorous DFT and (2) we evaluate performance pySCF's dimer calculation.
...
Our HOMO-LUMO gap ranges between $7.27eV$ and $6.34eV$. Consistent with our findings in \autoref{itic}, the
wall time for these individual calculations were on the order of seconds. 
...
Two thiophene rings are positioned in electronic proximity using mbuild and saved to gsd. 
...
and two sets of 12 translations along the x-axis and z-axis (with $0\AA <r< 5\AA$) were defined. The cartesian
...
avereges to $~2s$ per oreintation. 
