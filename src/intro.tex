...
in these materials is relatively narrow ($\sim300nm$).
...
relative permitivitty, $\epsilon_{r}\sim3$, in OPVs ($\epsilon_{r}\sim12$ for silicon)
...
This designed modularity is a benefit to researchers, as they can alter one component of the molecule and test
...
the end groups of the ITIC molecule. Using ab initio DFT, they found 
...
%ACTION: added a bit about solution processabilty

Our work in this thesis
is most expressly related to stage $4$ in that we seek to simulate the self-assembly and
...
The movement of charges through pure domains can be modeled as a particle hopping 
to and from nearby frontier molecular orbitals throughout the morphology.
...
In \autoref{results} we explore the delineation of these delocaliztion segments, which we hereafter refer to as a
``chromophore.'' 
...
wave function of an electron (hole) is thought to be fully delocalized in a pure acceptor (donor) domain. 
...
mirrored in the software used to study them. Ab initio DFT methods are prohibitivly slow on the nanometer
...
% ACTION : Made a figure and now need an appropriate paragraph to accompany it. 

With the pipeline laid out in \autoref{pipeline}, we can take atomistic representations like those shown in
\autoref{molecules} and, through a series of computations and simulations, arrive at data that is predictive
of material properties. This pipeline is laid out in further detail by Jones et al.~\cite{Jones2017} 
To begin, from an atomistic description of a given molecule, 
equillibrium molecular dynamics (MD) simulations can predict the morphological structure. 
The data obtained from these simulations can then be fed as an input into kinetic Monte
Carlo (KMC) simulations that characterize charge mobility (conductivity) in these chemistries based on the.
We introduce MD and our software stack used to implement it in \autoref{results}. 
The focus of this thesis, however, involves
...
\begin{figure}
  ...
  \includegraphics[width=0.9\linewidth]{figures/the-pipeline.png} 
    ...
    we see our packages developed for the facilitating these legs of the pipeline.}
  \label{pipeline}
\end{figure}
