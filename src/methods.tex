...
into the lowest unoccupied molecular orbital (LUMO) of $j$, and out if the
highest occupied molecular orbital (HOMO) of $i$.  

The driving force for a one electron charge transfer reaction, 
with rate is described by \autoref{marcus}, is the difference between the energy that our electron 
currently posseses 
on $i$, and the energy that it could enjoy over on chromophore $j$. 
...
Estimating these values requires a quantum chemical treatment. 
...
the dimer splitting methed [REFS]. This method comares the difference in energies between an electron
occupying either chromophore in isolation,
$\Delta E_{ij}$, to the energy of the electron occupying the frontier molecular orbitals of a dimer
consisting of the two chromophores. 
This difference is written as $E_{homo,dimer} - E_{homo-1,dimer}$ where $E_{homo,dimer}$ 
and $E_{homo-1,dimer}$ are the two highest energy occupied molecular orbitals of the dimer.

I found the
intuition for this method to be elusive. However, if we imagine
taking the dimer and pulling it apart, the two highest occupied energy levels of the dimer 
will become the HOMO of their respective chromophore. If the chromophores are not interacting, 
...
\begin{align}
    T_{ij} = \frac{1}{2}\sqrt{(\Delta E_{ij})^{2} - (E_{homo,dimer} - E_{homo-1,dimer})^{2}}
\end{align}

Solving shrodingers equation across an entire molecular arrangment. tens of thousands of 
