...
$\Delta_{ij}$, $T$ represent th electronic overlap, the reorganization energy, the free energy difference, and
...
The accuracy of the marcus rate is thus dependent on the accuracy with which the inputs can be estimated. 
But taking $k_{ij}$ to be and
...
\end{align}
where x is a random number between 0 and 1 and $\log{\frac{1}{x}}$ is a scaling factor. To illucidate this
graphically, $\ln{\frac{1}{x}}$ is plotted in figure \ref{fig:ln} from 0 to 1. 
...
The shortest waittime (quickest available hop) is chosen from the updated queue and the charge is moved to
its new chromophore host and $\tau$ is added to the time elapsed. Once the carrier has hopped beyond its
prespecified lifespan, its displacement is stored in the carrier object. After repeating this over a
statistically significant number of carriers, the carrier data can be aggregated and the mean squared
displacement can be calculated. It is known that the MSD of a diffusive particle increases linearly as time goes to infinity. 
The slope of the MSD in the linear regime gives the diffusion coefficient which can be used to determine the zero-field mobility using the following Einstein-Smoluchowski relation:
...
constant and $T$ is temperature. There is no objective best practice for determing the slope of the MSD as
time goes to inifinty from simulation data of this kind \cite{Maginn2018}. MorphCT requires the specification
of "lifetimes," which effectively serve as checkpoints at which the MSD of charge carriers is recorded. For
each "lifetime" the specified number of carrier objects to be simulated hop until they exceed their
"lifetime." The averages of these are recorded and a linear regression is performed to determine the slope
of the MSD. The sensitivity of the algorithm to this input is explored in the results section.  
