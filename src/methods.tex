...
\end{subfigure}
    \caption{Left: 1000 molecule atomistic morhology of ITIC. Right: 1000 oligomer atomistic simulation
    of P3HT}
\label{ITIC/P3HT}
\end{figure}
...
the \glsxtrshort{kmc} results to the choice or computaion of these parameters individually.
...
the dimer splitting methed~\cite{Huang2005b}. This method comares the \glsxtrshort{homo} energies of chromophores $i$ and
...
\glsxtrshort{homo} of chromophore $j$. If there is electronic overlap, a comparison bewteen the two highest occupied
...
application of a \glsxtrshort{kmc} algorthim.
...
A core tenet of Monte carlo samling is ensuring a large sample size. For this reason, we average over
...
How we analyse and aggregate data from thousands of
...
\texttt{MorphCT} is a package for running \glsxtrshort{kmc} simulations of charge transport in organics systems.
\texttt{MorphCT} operates on data containing the atom types and cartesian coordinates in a
system. We operate \texttt{MorphCT} on equilibrium microstates obtained from MD simulations. However,
...
System object. The System object can be instantiated from a GSD file. Methods
for adding chromophores, computing chromophore energies, and running
...
simulated across all available cores.
