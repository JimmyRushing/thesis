...
Assuming a one electron charge transfer reaction, the driving force for 
the reaction, $\Delta E_{ij}$, is the difference between the energy that it currently posseses 
on $i$, and the energy that is will posses over on chromophore $j$. 
is the energy offset between chromophore neighbors, which
