...
MorphCT is developed to faciliate charge cahracterization of morphological data. It takes GSD files which are
...
implemented this via a scaling of the wait times, or inverse hop rate, in the queue.
This rationally reshuffles the queue of events and introduces
significant stochasticism. The wait time for every potential hop from occupied chromophore $i$ onto a
...
\begin{figure}
  ...
  \caption{Graphical insight into the reshuffling of the wait time queue. Here the x-axis represents the 
    interval from which numbers are drawn randomly such that the wait times in the queue can be shifted
    by ln(1/x).}
  \label{fig:ln}
\end{figure}
...
prespecified lifespan, its displacement is stored in the carreir object. After repeating this over a
statistically significant number of carriers, the carrier data can be aggreggated and the mean squared
displacent can be calculated. It is known that the MSD of a diffusive particle increases linearly as time goes to infinity. 
...
time goes to inifinity from simulaton data of this kind \cite{Maginn2018}. MorphCT requires the specification
of "lifetimes," which effectivly serve as checkpoints at which the MSD of charge carriers is recorded. For
...
of the MSD. The senstivity of the algorithm to this input is explored in the results section.  
