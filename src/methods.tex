...
\ej{Cite some of the OG papers here, like LJ simulation of Argon.}
...
\ej{Check that the MTK integrator in HOOMD implements Nose-Hoover, then cite}
...
\ej{Expand explanation and cite Jankowski and/or Chodera papers that describe correlation times and independent samples. }

We procured the P3HT MD data used in this work off-the-shelf \cite{P3HTData}. \ej{Feels a little out of place to start this paragraph with where we got some data. What data? Forcefield files? log files? Trajectories? WHere is this paragraph going?}
This data was provided by researchers for validation and research purposes \cite{Miller2018}. 
Using coarse-graining techniques, wherein molecular segments are unified and treated as individual beads can lower the computational cost and allow for larger and longer simulations. \ej{Not the right place to unpack what CG is and why good}
...
In this case, the fuel for the iterative Newtonian update of positions and velocities was the Optimized Potentials for Liquid Simulations United Atom (OPLS-UA) force field.\ej{polish. is OPLS-UA really the ``fuel''?}

It is known about P3HT that it can have vastly different structures based on how it has been processed. \ej{Out of place in MD discussion? Structure-wise, you've introduced why MD is important, a bit about what MD is, but we don't yet have a description of the models of P3HT and ITIC explicitly stated here. OHHH, OK, so there are bits of the previous paragraph that are trying to get at what should be here, which is: Pictures of P3HT and ITIC, a description of the atom types, bonds, angles, and dihedrals that are used to model these molecules in MD, and then a generic description of initializing and equilibrating morphologies. THEN we can talk about how we didn't perform new P3HT simulations and DID perform new ITIC simulations, but that description belongs more in results than Methods.}
Three P3HT morphologies are studied in \autoref{results}. These morphologies have been coerced into various levels of crystallinity through a simulated anhealing process. We apply our workflow to a disordered, semi-crystalline, and crystalline sample.\ej{Why we talkin' results in Methods? Seems like we're telling ourselves to move this to Results, which we should!}
