...
\texttt{Planckton} is a conveneince package that integrates, \texttt{Mbuild}, \texttt{foyer},
\texttt{HOOMD-Blue} for performing MD simulations of self-assembly in OPVs. At current, \texttt{Planckton}
provides clearly documented template scripts for inititializing and runnning MD simulations and can be
implelmened with GAFF or OPLS forcefields. Also provided are files that define the atomic structure of many of
the most commonly studies OPVs for OSC reseach in a format that are compatible with GAFF forcefields. 
...
\texttt{Planckton-flow} allowed for a container of \texttt{Planckton} to be taken 
off the shelf an pulled to a the performance computing cluster without having to build the software stack or write the
simulation scripts from scratch.



\texttt{Planckton} is compatable with the forcefield used to generate the ITIC data; the Generalized
