...
%% JF this first sentence is a little jarring to me. We are not modeling charge hopping as two parabolas... We are using a frame from a simulation trajectory and the (atom) particles do not move for the duration of the kmc simulation (born-oppenheimer approximation). We use the energies obtained from quantum chemical calculations of pairs and single chromophores to information the rate of the carrier "hop" between each chromophore. I might reorganize this section a little to take up where you left off with the MD section. MD gives us a sampling of the equilibrium thermodynamic state, then we use one frame from that sample, we pick the chromophores, we calculate the energies, etc.
...
In organic systems, charges are localized due to the disorder in the available electronic states.
The spacial extent of chromophores is therefore dependent on the local environment of individual chromophores
and correspond roughly to the boundary between strongly interacting molecular regions and weakly interacting
ones. 
...
facilitating these two legs of the pipeline, \texttt{Planckton} and \texttt{MorphCT}, are maintained at 
the CMElab github repository \cite{cmelab}. As with any open-source software project, these repositories serve as a
central hub for developers to collaberate and integrate code into the code base. 
